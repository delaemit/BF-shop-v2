...
class ProductTableSeeder extends Seeder
{
    ...
    public function run($parameters = []): void
    {
        ...
        $products = Arr::map($localeProductsData[$defaultLocale], fn($row) => Arr::only($row, ['parent_id', 'sku', 'type', 'attribute_family_id', 'created_at', 'updated_at']));
        ...
        foreach ($localeProductsData as $locale => $productsData) {
            $productsFlatData = Arr::map($localeProductsData[$locale], fn($row) => Arr::except($row, ['color', 'size', 'parent_id']));
            ...
            foreach ($productsData as $productData) {
                foreach ($productData as $attributeCode => $value) {
                    ...
                    if ($locale !== 'en' && !in_array($attributeCode, $localeSpecificAttributes, true)) {
                        continue;
                    }
