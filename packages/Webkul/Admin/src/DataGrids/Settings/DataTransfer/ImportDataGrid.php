...
class ImportDataGrid extends DataGrid
{
    ...
    public function prepareColumns(): void
    {
        ...
        $this->addColumn([
            ...
            'type' => 'string',
            'closure' => fn($row) => '<a href="' . route('admin.settings.data_transfer.imports.download', $row->id) . '" class="cursor-pointer text-blue-600 hover:underline">' . $row->file_path . '<a>',
        ]);
    ...
    public function prepareActions(): void
    {
        if (bouncer()->hasPermission('settings.data_transfer.imports.import')) {
            $this->addAction([
                ...
                'method' => 'GET',
                'url' => fn($row) => route('admin.settings.data_transfer.imports.import', $row->id),
            ]);
        }
        ...
        if (bouncer()->hasPermission('settings.data_transfer.imports.edit')) {
            $this->addAction([
                ...
                'method' => 'GET',
                'url' => fn($row) => route('admin.settings.data_transfer.imports.edit', $row->id),
            ]);
        }
        ...
        if (bouncer()->hasPermission('settings.data_transfer.imports.delete')) {
            $this->addAction([
                ...
                'method' => 'DELETE',
                'url' => fn($row) => route('admin.settings.data_transfer.imports.delete', $row->id),
            ]);
        }
    }
}
