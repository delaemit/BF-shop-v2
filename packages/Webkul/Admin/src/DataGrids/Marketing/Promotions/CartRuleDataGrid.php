...
class CartRuleDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder()
    {
        $queryBuilder = DB::table('cart_rules')
            ->distinct()
            ->leftJoin('cart_rule_coupons', function ($leftJoin): void {
    ...
    public function prepareColumns(): void
    {
        ...
        $this->addColumn([
            ...
            'filterable' => true,
            'sortable' => true,
            'closure' => fn($value) => $value->coupon_code ?? '-',
        ]);
    ...
    public function prepareActions(): void
    {
        if (bouncer()->hasPermission('marketing.promotions.cart_rules.edit')) {
            $this->addAction([
                ...
                'method' => 'GET',
                'url' => fn($row) => route('admin.marketing.promotions.cart_rules.edit', $row->id),
            ]);
        }
        ...
        if (bouncer()->hasPermission('marketing.promotions.cart_rules.copy')) {
            $this->addAction([
                ...
                'method' => 'GET',
                'url' => fn($row) => route('admin.marketing.promotions.cart_rules.copy', $row->id),
            ]);
        }
        ...
        if (bouncer()->hasPermission('marketing.promotions.cart_rules.delete')) {
            $this->addAction([
                ...
                'method' => 'DELETE',
                'url' => fn($row) => route('admin.marketing.promotions.cart_rules.delete', $row->id),
            ]);
        }
    }
}
